...
namespace newLive
{
    ...
    {
        private byte[] _permutationTable = new byte[1024];
        private Random _rand = new Random();
        private const float K = 0.0105f;

        public PerlinNoise()
        {
            _rand.NextBytes(_permutationTable);
        }
        ...
        static float CubicCurve(float t)
        {
        ...
        private float NoiseWithoutOctaves(float fx, float fy)
        {
            ...
            pointInQuadX = CubicCurve(pointInQuadX);
            pointInQuadY = CubicCurve(pointInQuadY);
        ...
        public float Noise(float fx, float fy, int octaves, float persistence = 0.5f)
        {
            ...
            {
                max += amplitude;
                result += NoiseWithoutOctaves(fx, fy) * amplitude;
